...
is_criticality_balanced(500, 888)
...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    percentage_per_value = float((generated_power/theoretical_max_power) * 100)

    if percentage_per_value >= 80:
        return 'green'
    elif percentage_per_value >= 60 and percentage_per_value < 80:
        return 'orange'
    elif percentage_per_value >= 30 and percentage_per_value < 60:
        return 'red'
    else:
        return 'black'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    ninty_percent_threshold =(threshold * 90)/100
    ten_percent_threshold =(threshold * 10)/100
    min_value_trust = threshold - ten_percent_threshold
    max_value_trust = threshold + ten_percent_threshold
    danger = temperature * neutrons_produced_per_second

    if danger < ninty_percent_threshold:
        return 'LOW'
    elif min_value_trust <= danger <= max_value_trust :
        return 'NORMAL'
    else:
        return 'DANGER'
